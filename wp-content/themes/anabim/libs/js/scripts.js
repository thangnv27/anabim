...
jQuery(document).ready(function($){
    ...
    $('input.order-datepicker, .membership-datepicker').datepicker({ dateFormat: 'yy-mm-dd' });
    $(".order-update-expire").click(function (){
        var expire_date = $('input.order-datepicker').val().trim();
        if(expire_date.length === 10){
            window.location = $(this).parent('form').attr('action') + "&expire_date=" + expire_date;
        }
    });
