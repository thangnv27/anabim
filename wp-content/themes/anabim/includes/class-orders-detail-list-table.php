...
HTML;
            if($ordersRow->status == UNAPPROVED){
                echo '<a href="?page=nvt_orders" class="button">Quay lại</a>  <a href="'.$approveLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Duyệt</a>  <a href="'.$cancelLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Hủy</a>';
            }else if($ordersRow->status == APPROVED){
                echo '<a href="?page=nvt_orders" class="button">Quay lại</a>  <a href="'.$unapproveLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Không duyệt</a>  <a href="'.$cancelLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Hủy</a>';
            }else if($ordersRow->status == CANCELLED){
                echo '<a href="?page=nvt_orders" class="button">Quay lại</a>  <a href="'.$approveLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Duyệt</a>  <a href="'.$restoreLink.'" class="button" onclick="return confirm(\'Bạn có chắc chắn không?\');">Phục hồi</a>';
            }
//            echo '<a href="' . $printLink . '" target="_blank" class="button" onclick="window.open(\'' . $printLink . '\', \'windowname1\', \'width=420,height=600,top=0,left=0,scrollbars=1\'); return false;">In hóa đơn</a>';
    ...
    function get_columns() {
        return $columns = array(
            'col_orders_id' => __('ID'),
            'col_orders_name' => __('Khoá học'),
            'col_orders_amount' => __('Chi phí'),
        );
    }
    ...
    function prepare_items() {
        ...
        if(isset($_GET['expire_date'])){
            ...
            $sql_update = "UPDATE $tblOrders SET status = 1, expire_date = '$_expire_date' WHERE ID = $order_id";
        ...
        $expire_date = date('d/m/Y', strtotime($ordersRow->expire_date));
        $coupon_code = $ordersRow->coupon_code;
        $coupon_name = "";
        ...
        <h3>Mã giảm giá: {$ordersRow->coupon_code}</h3>
        <h3>Chiến dịch giảm giá: {$coupon_name}</h3>
        <h3>Số tiền thanh toán: <font color='red'>$total_amount</font> đ</h3>
        <h3>Ngày hết hạn: {$expire_date}</h3>
HTML;
