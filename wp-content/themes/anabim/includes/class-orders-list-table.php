...
class WPOrders_List_Table extends WP_List_Table {
    ...
    function get_columns() {
        return $columns = array(
            ...
            'col_orders_courses' => __('Khoá học'),
            'col_orders_date' => __('Ngày đăng ký'),
            'col_orders_expire' => __('Ngày hết hạn'),
    ...
    public function get_sortable_columns() {
        return $sortable = array(
            ...
            'col_orders_expire' => array('expire_date', false),
    ...
    function prepare_items() {
        ...
        if (isset($_GET['action'])) {
            ...
            wp_redirect(admin_url("admin.php?page=nvt_orders"));
            exit();
        }
...
//                $query .= " AND customer_info REGEXP '.*{\"fullname\":\"$search_query\",.*'";
                $query .= " AND customer_info LIKE '%$search_query%'";
            } else {
//                $query .= " WHERE customer_info REGEXP '.*{\"fullname\":\"$search_query\",.*'";
                $query .= " WHERE customer_info LIKE '%$search_query%'";
            }
        }
...
//                    $printLink = get_page_link(get_option(SHORT_NAME . '_pagePrintOrdersID')) .'/?order_id=' . (int) $rec->ID;
                    ...
                    switch ($column_name) {
                        ...
                        case "col_orders_customer_name": echo '<td ' . $attributes . '>' . ($rec->display_name ? $rec->display_name : "Guest") . '</td>';
                            break;
                        case "col_orders_courses":
                            echo '<td ' . $attributes . '>';
                            $products = json_decode($rec->products);
                        ...
                        case "col_orders_date": echo '<td ' . $attributes . '>' . $rec->created_at . '</td>';
                            break;
                        case "col_orders_expire": echo '<td ' . $attributes . '>' . date('d/m/Y', strtotime($rec->expire_date)) . '</td>';
                            break;
